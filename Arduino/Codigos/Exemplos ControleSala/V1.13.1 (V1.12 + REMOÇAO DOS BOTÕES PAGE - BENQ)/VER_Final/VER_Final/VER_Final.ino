#include <SoftwareSerial.h> //Inclui a função para utilizar outros inputs como Serial
#include <IRremote.h> //inclui a função InfraVermelho
#include <virtuabotixRTC.h> //inclui a função para o RTC(Real Time Clock)
#include <TimerOne.h> //inclui a função timer
#include <avr/wdt.h> //inclui a função watchdog

virtuabotixRTC myRTC(6, 7, 8); // define quais serão os pinos de conexão com o RTC DS-1302
SoftwareSerial serial1(10, 11); // RX, TX, define quais serão os novos pinos de comunicaçao serial, sera utilizado no bluetooth

IRsend irsend; // define um novo nome para o InfraVermelho

const int LED = 13;

int indice1=-1;
int indice3=-1;
int horadesliga=22; //hora programada para desligamento
int mindesliga=00; //minuto programado para desligamento
int secatu=00; //segundo atual
int minatu=00; //minuto atual
int horaatu=00; //hora atual
...
void setup()
{
  ...
  //myRTC.setDS1302Time(00, 00, 16, 2, 25, 9, 2017); //segundo, minuto, hora, dia da semana, dia, mes, ano
  //delay(1000);
  serial1.begin(38400);
  //Serial.begin(38400);
  valor1[0]=0;
...
//-------------------------------------------------------------------------------------------
  if ((ACLIG==true) && (contadortimerAC>=122) && (auxcontadortimerAC==true))
    {
...
//-------------------------------------------------------------------------------------------
  if (((myRTC.hours==22) && (myRTC.minutes==0) && (myRTC.seconds==0)) || ((myRTC.hours==22) && (myRTC.minutes==0)&& (myRTC.seconds==1)))
    {
      unsigned int irSignal[] = {6280, 7580, 560, 580, 600, 520, 580, 560, 580, 520, 620, 520, 560, 560, 600, 520, 600, 540, 580, 580, 580, 540, 560, 560, 600, 540, 580, 540, 600, 520, 600, 540, 560, 560, 600, 560, 520, 600, 600, 520, 600, 540, 580, 540, 600, 540, 540, 580, 580, 540, 580, 580, 580, 540, 600, 540, 540, 560, 620, 540, 560, 540, 560, 560, 600, 540, 520, 640, 580, 560, 560, 540, 600, 520, 600, 540, 560, 560, 600, 540, 580, 540, 560, 600, 580, 540, 560, 560, 600, 540, 520, 600, 600, 520, 600, 540, 540, 580, 600, 560, 500, 620, 600, 520, 600, 540, 560, 560, 600, 540, 500, 600, 600, 520, 620, 560, 580, 540, 600, 540, 500, 1720, 580, 1640, 580, 1640, 600, 1640, 540, 560, 600, 580, 560, 560, 600, 540, 580, 520, 600, 520, 600, 540, 540, 580, 600, 520, 600, 1660, 600, 540, 500, 620, 600, 1620, 580, 540, 600, 540, 520, 600, 600, 1640, 540, 1720, 500, 600, 600, 1640, 580, 540, 600, 540, 500, 1720, 580, 540, 560, 1660, 520, 640, 600, 1620, 600, 1640, 500, 620, 580, 560, 560, 1640, 600, 540, 500, 1720, 580, 7580, 580};
      //Serial.println("DESLIGAR AC");
      irsend.sendRaw(irSignal, sizeof(irSignal) / sizeof(irSignal[0]), khz);
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
          case 'B':     //caso 'B' = 66
            {
              irsend.sendNEC(0xC40BF, 32);    //manda o sinal IR-LIGA/DESLIGA
              delay(1000);
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
          case 'C':       //caso 'C' = 67
            {
              irsend.sendNEC(0xCC03F, 32);    //manda o sinal IR-FREEZE
              break;
            }
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
          case 'D':       //caso 'D' = 68
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
          case 'G':       //caso 'G' = 71
            {
              irsend.sendNEC(0xC08F7, 32);    //manda o sinal IR-ENTER
              break;
            }
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
          /*case 'H':       //caso 'H' = 72
